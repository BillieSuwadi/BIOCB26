...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--convert_dir", help="Path to the input folder")
    parser.add_argument("--count_dir", help = "Path to count .nii.gz files")
...
def main() -> None:
    ...
    if args.convert_dir is not None:
        root_dir = Path(args.convert_dir).expanduser().resolve()
    ...
    if args.count_dir is not None:
        root_dir = Path(args.count_dir).expanduser().resolve()
